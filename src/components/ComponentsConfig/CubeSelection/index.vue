<template>
  <div>
    ...
    <div class="com-group-magic">
      <div class="magic-box" :class="selectStyle.id">
        <div
          ...
          :class="[
            'magic-' + (index + 1),
            { nodata: !item.imageUrl, active: activeIndex === index },
          ]"
    ...
    <ComGroup
      title="选择模板"
      :content-block="true"
      :select-value="selectStyle.name"
    >
      <ComRadioGroup
        v-model="configData.template"
        :radio-list="styleList"
        :content-block="true"
      />
    </ComGroup>
  ...
  computed: {
    ...
    selectStyle() {
      return this.styleList.find(
        (item) => item.id === this.configData.template
      );
    },
    selectPic: {
      get: function () {
