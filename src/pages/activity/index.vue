<template>
  ...
  >
    <!-- 搜索按钮区域 -->
    <template slot="isCompSearch"> </template>
    <!-- 操作表格区域 -->
    <template #handleDetail="{ row }">
      <el-button type="text" size="mini" @click="onEdit(row.id)">
        编辑
      </el-button>
      <el-button size="mini" type="text" @click="onToggleActivity(row)">
        {{ row.isAbled === 0 ? "上线" : "下线" }}
      </el-button>
      <el-button size="mini" type="text" @click="onPreview(row.id, row.title)">
        预览
      </el-button>
  ...
  methods: {
    ...
    onPreview(id) {
      let url = this.$router.resolve({
        path: `/preview`,
        query: { id, data: this.$store.state.wxParams },
      });
